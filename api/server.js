...
server.get("/api/dogs/:id", async (req, res) => {
    try{
        ...
        res.status(500).json({message:`Error fetching dogs ${req.params.id}: ${err.message}`})
    }
})
...
server.post("/api/dogs", async (req, res) => {
    try{
        ...
        if (!name || !weight) {
            res.status(422).json({message: "dogs need name and weight"})
        } else {
            const createdDog = await Dog.create({name, weight})
            res.status(201).json({message: "success creating dog", createdDog})
        }        
    } catch(err) {
    ...
    try{
        ...
        if (!name || !weight) {
            ...
            if (!updatedDog) {
                ...
                res.status(200).json({message: "dog was updated successfully",
                data: updatedDog})
            }
        }
...
server.delete("/api/dogs/:id", async (req, res) => {
    try{
        ...
        if (!deletedDog) {
            res.status(404).json({message: `Dog id ${id} not found`})
        } else {
